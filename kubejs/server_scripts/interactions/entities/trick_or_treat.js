...
ItemEvents.rightClicked('minecraft:wooden_axe', e => {
	...
	let ObjectiveCriteria = Java.loadClass(
	...
	ItemEvents.entityInteracted("minecraft:bundle", (event) => {
		...
		if (isTrickOrTreatTime(currentTime)) {
			...
			if (player.stages.has(clicked)) {
				player.tell(
					`You have already received a Trick or Treat from this creature type.`
				);
				return;
			}
			let score = getPlayerTrickOrTreatScoreReference(player);
			...
			event.server.scheduleInTicks(1.5 * 20, (callback) => {
				...
				if (rng <= 0.25 && rng >= 0.0) {
					if (subRng <= 0.2) {
						...
						player.give(
							Item.of(
								...
								'{display:{Lore:[\'["",{"text":"Aww, I got a rock...","italic":false,"color":"yellow"}]\'],Name:\'["",{"text":"Rock","italic":false}]\'}}'
							)
						);
						return;
					}
					if (subRng > 0.2 && subRng <= 0.4) {
						player.tell("...trick! Enjoy your new dirt!");
						player.give("minecraft:dirt");
						return;
					}
					if (subRng > 0.4 && subRng <= 0.6) {
						player.tell("...trick! Enjoy fighting the Warden!");
						event.server.runCommandSilent(
							`playsound minecraft:entity.warden.emerge player ${event.player.name.string} ${x} ${y} ${z} 10`
						);
					...
					if (subRng > 0.6 && subRng <= 0.8) {
						player.tell("...trick! Enjoy your flight!");
						event.server.runCommandSilent(
							`effect give ${event.player.name.string} minecraft:levitation 10 1`
						);
						return;
					}
					if (subRng > 0.8 && subRng <= 0.9) {
					...
					if (subRng > 0.9 && subRng <= 1.0) {
						...
						event.server.runCommandSilent(
							`playsound minecraft:entity.creeper.primed player ${event.player.name.string} ${x} ${y} ${z} 10`
						);
						event.server.runCommandSilent(
							`summon sheep ${player.x} ${player.y} ${player.z} {Color:13,CustomName:'[{"text":"Dinnerbone"}]',Silent:1b}`
						);
						return;
					}
				}
