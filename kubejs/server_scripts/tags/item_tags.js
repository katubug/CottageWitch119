/**
 * Generally adding tags to items
 */
ServerEvents.tags("item", (event) => {
	//////////////////////////////////////////////////

	// #region NATURAL ITEMS
	// Anything that fits a natural vibe
	// Trying to section this out to make it easier to navigate
	...
	// Log Tags
	event.add("forge:azalea_log", /.*azalea_log.*/);
	event.add("forge:stripped_azalea_log", /.*stripped_azalea_log.*/);

	// Wood Tags
	event.add("forge:azalea_wood", /.*azalea_wood.*/);
	event.add("forge:stripped_azalea_wood", /.*stripped_azalea_wood.*/);

	// Planks Tags
	event.add("forge:azalea_planks", /.*azalea_planks.*/);

	// Remove quark ones
	event.remove("forge:azalea_log", /quark.*azalea_log.*/);
	event.remove("forge:stripped_azalea_log", /quark.*stripped_azalea_log.*/);

	event.remove("forge:azalea_wood", /quark.*azalea_wood.*/);
	event.remove("forge:stripped_azalea_wood", /quark.*stripped_azalea_wood.*/);

	event.remove("forge:azalea_planks", /quark.*azalea_planks.*/);
	...
	//MARK:| BYG Sand Tags
	event.add("forge:sand/purple", "byg:purple_sand");
	event.add("forge:sand/pink", "byg:pink_sand");
	event.add("forge:sand/blue", "byg:blue_sand");
	event.add("forge:sand/white", "byg:white_sand");
	event.add("forge:sand/black", "byg:black_sand");
	...
	// #region FOODS
	// This covers everything food related, unsurprisingly.
	// Mainly forge food tags and caupona tags
	...
	// Bison Patties
	event.add("forge:cooked_beef", "alexsdelight:bison_patty");
	...
	event.add(
		"caupona:cereals/baked", //
		"culturaldelights:tortilla"
	);

	// Corn Dough
	event.add("minecraft:dough", "culturaldelights:corn_dough");
	event.add("forge:dough", "culturaldelights:corn_dough");
	...
	// This covers Stuff That Is Useful
	// Kinda anything that doesn't fit but has some kinda defined use
	...
	//MARK:| Blackstone Furnace
	event.add(
		"forge:blackstone_furnaces",
	...
	// Anything used in crafting recipes
	...
	// #region TOOLS
	// As expected, everything that is a tool
	// includes vehicular stuff and things you use
	...
	//MARK:| Aquaculture Hooks

	// Upgraded Hooks
	event.add(
		"forge:upgraded_hooks",
	...
	const toolsToTag = {
		all: [
			...
			"byg:pendorite",
			"gildednetherite:gilded_netherite",
			"deeperdarker:warden",
			"experienceobelisk:cognitive",
		...
		axe: [
			"twilightforest:golden_minotaur_axe",
	...
	const tagsForTools = {
		...
		hoe: [
			"overweight_farming:overweight_harvestables",
			"create:upright_on_deployer",
			"forge:hoes",
			"forge:tools",
			"forge:tools/hoes",
	...
	for (let group of Object.keys(toolsToTag)) {
		console.log("Group: " + group);
		if (group == "all") {
			for (let toolType in tagsForTools) {
				// get list of tools for each type
				let toolList = toolsToTag.all.map((base) => {
					return `${base}_${toolType}`;
				});
				console.log(`ALL - Adding ${toolType} tags to ${toolList.length} items`);
				// add tags in one go
				console.log(tagsForTools[toolType]);
				for (let tag of tagsForTools[toolType]) {
					event.add(tag, toolList);
				}
			}
		} else {
			let toolList = toolsToTag[group];
			console.log(`UNIQUE - Adding ${group} tags to ${toolList.length} items`);
			for (let tag of tagsForTools[group]) {
				event.add(tag, toolList);
			}
		}
	}
	...
	// #region SOPH STORAGE
	// Grouping together of all
	// the soph storage stuff

	//MARK:| Inventory Managers
	event.add(
		"forge:inventory_controllers", //
		"sophisticatedstorage:controller"
	);

	//MARK:| Inventory Upgrades
	event.add(
		"forge:inventory_upgrades",
	...
	// #region SOPH BACKPACKS
	// Grouping together of all
	// the soph backpacks stuff
	...
	// #region OTHER
	// Anything that doesn't really fit
	// into the rest of the groups
